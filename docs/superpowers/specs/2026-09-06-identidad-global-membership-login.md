...
- Los campos `identityProvider`, `identityIssuer`, `identitySubject`, `emailClaim` se eliminan de TODA la cadena de creación y consulta de usuario, no solo del command: `CrearUsuarioRequest` (HTTP), `CrearUsuarioCommand`, `UsuarioResult`, `UsuarioResponse` (HTTP), `UsuarioProjection` (lectura vía `IamJdbcReadRepository.findUsers`/`USER_FILTER`, que hoy hace `LEFT JOIN sch_seguridad.identidad_externa` solo para exponer esos 4 campos — ese `LEFT JOIN` y sus columnas seleccionadas se eliminan de la query). Quedan en cada uno de esos tipos: `tenantId`, `documentType`, `documentNumber`, `firstNames`, `lastNames`, `username`, `email`, `phone`, `displayName`, `credentialChangeRequired`, `mfaRequired` (más `id`/`status`/`createdAt`/`updatedAt` donde ya existían). `IamApiMapper.toCommand`/`toResponse` y `IamApplicationMapper.toResult` se actualizan en consecuencia (dejan de leer `user.identity()...`).
